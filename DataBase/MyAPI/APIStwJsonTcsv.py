...
@app.route('/bigdata/user_file_product/getinfo',methods=['get'])    
def get_userinfo():
    stwdata=request.json.get('stwdata')
    #schoolid = str(request.json.get('schoolid')) #获取带json串请求的userid参数传入的值
    content=Tranjsoncsv.jsontocsv(stwdata)
    response = make_response(content)
    response.headers["Content-Disposition"] = "attachment; filename*=ANSI''myfilename.csv;"
    
    return response


app.run(port=8803,debug=True,threaded=True) 
...
class Tranjsoncsv(object):
    ...
    def jsontocsv(self):
        ...
        jsonvalues=json.loads(self.jsondata)
