...
class BaseViz(object):
    ...
    def query_obj(self):
        ...
        _role_list = ['班主任', '任课老师', '年级主任', '学科组长', '校长', ]
        user_role_type = g.user.roles

        def role_type_check():
            class_ids = g.user.class_ids
            subject_ids = g.user.subject_ids
            school_ids = g.user.school_ids
            fm_data = form_data.get('where', '')
            if fm_data:
                fm_data = fm_data + 'AND '
            my_path = '/test.log'

            if not subject_ids:
                if not class_ids and school_ids:
                    query_data = fm_data + ' SCHOOL_ID IN(%s)' % school_ids
                elif not school_ids and class_ids:
                    query_data = fm_data + ' CLASS_ID IN(%s)' % class_ids
                else:
                    query_data = fm_data + '1=1'
            elif not school_ids and class_ids:
                query_data = fm_data + ' CLASS_ID IN(%s) AND SUBJECT_ID IN (%s)' % (class_ids, subject_ids)
            elif not class_ids and school_ids:
                query_data = fm_data + ' SCHOOL_ID IN(%s) AND SUBJECT_ID IN (%s)' % (school_ids, subject_ids)
            else:
                query_data = fm_data + '1=1'
            with open(my_path, 'a') as f:
                print(query_data, class_ids, subject_ids, school_ids, file=f)
            return query_data

            # if '班主任' in user_roles or '年级主任' in user_roles:
            #     query_data = fm_data + ' AND CLASS_ID IN(%s)' % class_ids
            # elif '任课老师' in user_roles or '学科组长' in user_roles:
            #     query_data = fm_data + ' AND CLASS_ID IN(%s) AND SUBJECT_ID IN (%s)' % (class_ids, subject_id)
            # elif '校长' in user_roles:
            #     query_data = fm_data + ' AND SCHOOL_ID IN(%s)' % school_ids
            # else:
            #     query_data = fm_data + ' AND SCHOOL_ID IN(%s)' % school_ids

        where_data = role_type_check()

        extras = {
            'where': where_data,  # form_data.get('where', ''),
